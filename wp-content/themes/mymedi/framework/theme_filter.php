...
add_filter('authenticate', 'mymedi_login_failed_redirect', 30, 3);
function mymedi_login_failed_redirect( $user, $username, $password ){
		...
		if( function_exists('wc_get_page_permalink') ){
			...
			if( !empty($referer) && !strstr($referer, 'wp-login') && !strstr($referer, 'wp-admin') ){
				...
				if( is_a($user, 'WP_Error') ){
					...
					wc_add_notice( __('Username or password is incorrect', 'mymedi'), 'error' );
				}
